# Project log — how The Factory Floor was built

This file is the working history of the project: the dated session notes, the bootcamp
compliance checklist, and the presentation script that used to live at the top of
`README.md`. It is kept verbatim because the reasoning trail — including the things that
were tried and abandoned — is part of what the project is.

The README is now the front door (what it is, how to run it, what was measured).
`CLAUDE.md` holds the full per-session engineering notes; this file is the readable
middle layer between the two.

The notes below are in reverse-chronological blocks, exactly as they were written at the
time. Where one says "read this FIRST before continuing" or "resume point", read that as
*"as of that date"* — the current state of the project is the README.
...
## CLAUDE.md working notes, 2026-08-18 → 2026-09-04 (moved here verbatim on 2026-09-05)

On 2026-09-05 `CLAUDE.md` was cut down to the rules, traps and standing decisions a future
session actually needs. Everything it used to contain from its "What this project is" section
onwards is preserved below, unchanged and in its original order (which was not chronological:
the 2026-08-27/28 professionalization notes were written above the 2026-08-18 notes). Older
references in this text to `dificuldades_e_oportunidades.md` mean `docs/limitations_and_opportunities.md`;
references to "README's <date> note" mean the session notes earlier in this file.
...
## Limitations and opportunities — resolved entries (moved here verbatim on 2026-09-05)

`docs/limitations_and_opportunities.md` now lists only what is still open, plus a one-line table of
what was resolved and when. The full write-ups of every resolved item, including the original
diagnosis kept struck-through under each one, are preserved below unchanged. Item numbers are the
same as in that document and as referenced from code comments (`audit.py` #6, `agent.py` #18).
